...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Elven Discord Bot Launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Elven",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Elven in case of issues",
                        action="store_true")
    parser.add_argument("--update-elven",
                        help="Updates Elven (git)",
                        action="store_true")
...
def update_elven():
    try:
        ...
        return
    if code == 0:
        print("\nElven has been updated")
    else:
        print("\nElven could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")


def reset_elven(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if git_reset:
        ...
        if code == 0:
            print("Elven has been restored to the last local commit.")
        else:
...
def download_ffmpeg(bitness):
    clear_screen()
    repo = "https://github.com/EL-ElvenLord/ElvenBot"
    verified = []
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Elven:")
        print("1. Update Elven + requirements (recommended)")
        print("2. Update Elven")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Elven (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_elven(data=True)
                wait()
        elif choice == "3":
            reset_elven(reqs=True)
            wait()
        elif choice == "4":
            ...
            if user_pick_yes_no():
                reset_elven(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_elven(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Elven. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "elven.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Elven...")
                continue
            else:
                if not autorestart:
                    break

    print("Elven has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Elven without going
    ...
    start_elven = "{} --start".format(call)
    start_elven_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_elven             = ccd + start_elven             + pause
    start_elven_autorestart = ccd + start_elven_autorestart + pause

    files = {
        "start_elven"             + ext : start_elven,
        "start_elven_autorestart" + ext : start_elven_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Elven Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Elven has been "
        ...
        print("1. Run Elven /w autorestart in case of issues")
        print("2. Run Elven")
        print("3. Update")
        ...
        choice = user_choice()
        if choice == "1":
            run_elven(autorestart=True)
        elif choice == "2":
            run_elven(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Elven needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Elven cannot work without the pip module. Please make sure to "
              "install Python without unchecking any option during the setup")
        wait()
        exit(1)
    if args.repair:
        reset_elven(git_reset=True)
    if args.update_elven:
        update_elven()
    if args.update_reqs:
        install_reqs(audio=True)
    ...
    if INTERACTIVE_MODE:
        ...
        print("Starting Elven...")
        run_elven(autorestart=args.auto_restart)
